...
import type { Anmeldungen, Kurse, Teilnehmer, LookupValue } from '@/types/app';
...
export function AnmeldungenDialog({ open, onClose, onSubmit, defaultValues, recordId, kurseList, teilnehmerList, enablePhotoScan = true, enablePhotoLocation = true }: AnmeldungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['kurs', 'teilnehmer', 'anmeldedatum', 'status'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'kurs': (
      <div key="kurs" className="space-y-1.5">
        ...
        <Combobox
          id="kurs"
          placeholder="Für welchen Kurs anmelden?"
        ...
        {showErrors && !fields.kurs && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('anmeldungen', 'kurs')}</p>
        )}
      </div>
    ),
    'teilnehmer': (
      <div key="teilnehmer" className="space-y-1.5">
        ...
        <Combobox
          id="teilnehmer"
          placeholder="Welcher Teilnehmer meldet sich an?"
        ...
        {showErrors && !fields.teilnehmer && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('anmeldungen', 'teilnehmer')}</p>
        )}
      </div>
    ),
    'anmeldedatum': (
      <div key="anmeldedatum" className="space-y-1.5">
        ...
        {showErrors && !fields.anmeldedatum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('anmeldungen', 'anmeldedatum')}</p>
        )}
      </div>
    ),
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'neu'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'neu' ? undefined : 'neu') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'neu'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('anmeldungen', 'status', 'neu') ?? 'Neu'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('anmeldungen', 'status', 'bestaetigt') ?? 'Bestätigt'}
          </button>
          <button
            type="button"
            role="radio"
    ...
    'bezahlt': (
      <div key="bezahlt" className="space-y-1.5">
        <Label htmlFor="bezahlt">{fieldLabel('anmeldungen', 'bezahlt')}</Label>
        <div className="flex items-center gap-2 pt-1">
          ...
          />
          <Label htmlFor="bezahlt" className="font-normal">{fieldLabel('anmeldungen', 'bezahlt')}</Label>
        </div>
      </div>
    ),
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkung"
          placeholder="Anmerkungen zur Anmeldung — Allergien, Besonderheiten…"
  ...
  const FIELD_LABELS: Record<string, string> = {"kurs": "Kurs", "teilnehmer": "Teilnehmer", "anmeldedatum": "Anmeldedatum", "status": "Status", "bezahlt": "Bezahlt", "bemerkung": "Bemerkung"};
