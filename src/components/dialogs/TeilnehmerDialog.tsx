...
export function TeilnehmerDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: TeilnehmerDialogProps) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'vorname': (
      <div key="vorname" className="space-y-1.5">
        ...
        {showErrors && !fields.vorname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('teilnehmer', 'vorname')}</p>
        )}
      </div>
    ),
    'nachname': (
      <div key="nachname" className="space-y-1.5">
        ...
        {showErrors && !fields.nachname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('teilnehmer', 'nachname')}</p>
        )}
      </div>
    ),
    'geburtsdatum': (
      <div key="geburtsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="geburtsdatum"
          placeholder="Wann wurde der Teilnehmer geboren?"
          mode="date"
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        ...
        <Input
          ...
          type="email"
          inputMode="email"
          placeholder="z. B. anna.schmidt@example.de"
          value={fields.email ?? ''}
    ...
    'erziehungsberechtigte_person': (
      <div key="erziehungsberechtigte_person" className="space-y-1.5">
        <Label htmlFor="erziehungsberechtigte_person">{fieldLabel('teilnehmer', 'erziehungsberechtigte_person')}</Label>
        <Input
          id="erziehungsberechtigte_person"
          placeholder="Name des Erziehungsberechtigten"
          value={fields.erziehungsberechtigte_person ?? ''}
          onChange={e => setFields(f => ({ ...f, erziehungsberechtigte_person: e.target.value }))}
        />
      </div>
    ),
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        <Label htmlFor="notizen">{fieldLabel('teilnehmer', 'notizen')}</Label>
        <Textarea
          id="notizen"
          placeholder="Besonderheiten, allergische Reaktionen, musikalische Vorerfahrung…"
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "geburtsdatum": "Geburtsdatum", "email": "E-Mail", "telefon": "Telefon", "erziehungsberechtigte_person": "Erziehungsberechtigte Person", "notizen": "Notizen"};
