...
export type DozentenDialogDefaults = Dozenten['fields'];
...
export function DozentenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: DozentenDialogProps) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'vorname': (
      <div key="vorname" className="space-y-1.5">
        ...
        {showErrors && !fields.vorname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('dozenten', 'vorname')}</p>
        )}
      </div>
    ),
    'nachname': (
      <div key="nachname" className="space-y-1.5">
        ...
        {showErrors && !fields.nachname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('dozenten', 'nachname')}</p>
        )}
      </div>
    ),
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('dozenten', 'email')}</Label>
        <Input
          id="email"
          type="email"
          inputMode="email"
          placeholder="z. B. maria@klangraum.de"
          value={fields.email ?? ''}
    ...
    'instrumente': (
      <div key="instrumente" className="space-y-1.5">
        <Label htmlFor="instrumente">{fieldLabel('dozenten', 'instrumente')}</Label>
        <Textarea
          id="instrumente"
          placeholder="Ein Instrument pro Zeile — z. B. Klavier, Gitarre"
          value={fields.instrumente ?? ''}
          onChange={e => setFields(f => ({ ...f, instrumente: e.target.value }))}
          rows={3}
        />
      </div>
    ),
    'aktiv': (
      <div key="aktiv" className="space-y-1.5">
        <Label htmlFor="aktiv">{fieldLabel('dozenten', 'aktiv')}</Label>
        <div className="flex items-center gap-2 pt-1">
          ...
          />
          <Label htmlFor="aktiv" className="font-normal">{fieldLabel('dozenten', 'aktiv')}</Label>
        </div>
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail", "telefon": "Telefon", "instrumente": "Instrumente", "aktiv": "Aktiv"};
