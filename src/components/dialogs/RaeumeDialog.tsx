...
export function RaeumeDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: RaeumeDialogProps) {
  ...
  const REQUIRED_FIELDS = ['name', 'plaetze'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "name": string | null, // Name\n  "plaetze": number | null, // Plätze\n  "klavier_vorhanden": boolean | null, // Klavier vorhanden\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'name': (
      <div key="name" className="space-y-1.5">
        <Label htmlFor="name">{fieldLabel('raeume', 'name')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="name"
          placeholder="z. B. Studio 1, Saal B"
          value={fields.name ?? ''}
          onChange={e => setFields(f => ({ ...f, name: e.target.value }))}
          required
        />
        {showErrors && !fields.name && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('raeume', 'name')}</p>
        )}
      </div>
    ),
    'plaetze': (
      <div key="plaetze" className="space-y-1.5">
        <Label htmlFor="plaetze">{fieldLabel('raeume', 'plaetze')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="plaetze"
          type="number"
          inputMode="decimal"
          step="any"
          {...numberInputProps(formEnhancements, 'plaetze')}
          placeholder="z. B. 20"
          value={fields.plaetze !== undefined ? fields.plaetze : (computedValues['plaetze'] ?? '')}
          onChange={e => setFields(f => ({ ...f, plaetze: clampNumberValue(formEnhancements, 'plaetze', e.target.value) }))}
        />
        {showErrors && !fields.plaetze && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('raeume', 'plaetze')}</p>
        )}
      </div>
    ),
    'klavier_vorhanden': (
      <div key="klavier_vorhanden" className="space-y-1.5">
        <Label htmlFor="klavier_vorhanden">{fieldLabel('raeume', 'klavier_vorhanden')}</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="klavier_vorhanden"
            checked={!!fields.klavier_vorhanden}
            onCheckedChange={(v) => setFields(f => ({ ...f, klavier_vorhanden: !!v }))}
          />
          <Label htmlFor="klavier_vorhanden" className="font-normal">{fieldLabel('raeume', 'klavier_vorhanden')}</Label>
        </div>
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"name": "Name", "plaetze": "Plätze", "klavier_vorhanden": "Klavier vorhanden"};
