 ...
 *   - dozenten: no date pair, no references
 *   - raeume: no date pair, no references
 *   - kurse: date pairs beginn+ende · applookups dozent→dozenten, raum→raeume · lookups instrument[klavier|gitarre|violine|cello|querfloete|klarinette|schlagzeug|gesang|blockfloete], niveau[anfaenger|fortgeschritten|profi], status[geplant|laeuft|abgeschlossen|abgesagt]
 *   - teilnehmer: no date pair, no references
 *   - anmeldungen: applookups kurs→kurse, teilnehmer→teilnehmer · lookups status[abgemeldet|neu|bestaetigt|warteliste]
 *   - anwesenheiten: applookups kurs→kurse, teilnehmer→teilnehmer
 */
...
export const OCCUPANCY: Partial<Record<EntityKey, OccupancyRule>> = {
  // <custom:occupancy>
  kurse: { from: 'beginn', to: 'ende', resource: 'raum', statusField: 'status', freeKeys: ['abgesagt', 'abgeschlossen'] },
  // </custom:occupancy>
};
